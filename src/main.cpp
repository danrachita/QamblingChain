...
#if defined(NDEBUG)
#error "QamblingChain cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uqcn) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK())
        nSubsidy = 4800 * COIN;
    else if (nHeight <= 1640)
        nSubsidy = 1 * COIN;
    else if (nHeight > 1640 && nHeight <= 8840)
        nSubsidy = 60 * COIN;
    else if (nHeight > 8840 && nHeight <= 18920)
        nSubsidy = 50 * COIN;
    else if (nHeight > 18920 && nHeight <= 29000)
        nSubsidy = 44 * COIN;
    else if (nHeight > 29000 && nHeight <= 39080)
        nSubsidy = 38 * COIN;
    else if (nHeight > 39080 && nHeight <= 49160)
        nSubsidy = 32 * COIN;
    else if (nHeight > 49160 && nHeight <= 275000)
        nSubsidy = 25 * COIN;
    else if (nHeight > 275000 && nHeight <= 350000)
        nSubsidy = 20 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if (nHeight <= 275000)
        	ret = blockValue * 0.85; // 70% of block reward
	else
...
{
    RenameThread("qamblingchain-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // QCN: We use certain sporks during IBD, so check to see if they are
