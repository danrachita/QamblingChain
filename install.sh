...
QCN Masternode installer arguments:
...
echo "
 ...
 | the QCN Team CANNOT assist you in repairing  |::
...
echo "" && echo 'Added user "qamblingchain"' && echo ""
sleep 1
...
rm $TARBALLNAME
mv ./qamblingchaind /usr/local/bin
mv ./qamblingchain-cli /usr/local/bin
mv ./qamblingchain-tx /usr/local/bin
rm -rf $TARBALLNAME

# Create .qamblingchain directory
mkdir $USERHOME/.qamblingchain
...
# Create qamblingchain.conf
touch $USERHOME/.qamblingchain/qamblingchain.conf
cat > $USERHOME/.qamblingchain/qamblingchain.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
cat > /etc/systemd/system/qamblingchain.service << EOL
[Unit]
Description=qamblingchaind
After=network.target
[Service]
Type=forking
User=${USER}
WorkingDirectory=${USERHOME}
ExecStart=/usr/local/bin/qamblingchaind -conf=${USERHOME}/.qamblingchain/qamblingchain.conf -datadir=${USERHOME}/.qamblingchain
ExecStop=/usr/local/bin/qamblingchain-cli -conf=${USERHOME}/.qamblingchain/qamblingchain.conf -datadir=${USERHOME}/.qamblingchain stop
Restart=on-abort
[Install]
WantedBy=multi-user.target
EOL
sudo systemctl enable qamblingchain.service
sudo systemctl start qamblingchain.service
